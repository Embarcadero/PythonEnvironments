unit PyEnvironment.Project.ManagerMenu;
...
begin
  Result := FIsPyEnvironmentEnabled or TPyEnvironmentProject.Found;
end;
...
begin
  ...
  begin
    ...
    if AEnabled and (APlatform in TPyEnvironmentProject.SUPPORTED_PLATFORMS) then begin
      for LDeployFile in TPyEnvironmentProject.GetDeployFiles(LPythonVersion, APlatform) do
        TPyEnvironmentProjectHelper.AddDeployFile(AProject, AConfig, LDeployFile);
    end else begin
      for LDeployFile in TPyEnvironmentProject.GetDeployFiles(LPythonVersion, APlatform) do begin
